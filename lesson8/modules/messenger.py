...
class JimClient(Jim):
    ...
    def connect(self):
        try:
            ...
            self._logger.debug(f'Подключен к серверу {self.address}:{self.port}')
        except ConnectionRefusedError:
            self._logger.critical(f'Сервер {self.address}:{self.port}'
                                  f' отказался от подключения')
            sys.exit()
    ...
    def ok_handler(self, data: dict):
        try:
            code = data['code']
            alert = 'no additional message provided' if len(data['alert']) == 0 else 'message: ' + data['alert']
    ...
    def error_handler(self, data: dict):
        try:
            code = data['code']
            error = data['error']
            self._logger.error(f'Ошибка ({code}): {error}')
            return code
        except (KeyError, ValueError):
            self._logger.critical(INVALID_JSON)
            self.client_sock.close()
...
class JimServer(Jim):
    ...
    def __init__(self, address: str, port: int, logger=EmptyLogger()) -> None:
        ...
        self._logger.info(f'Сервер создан и будет прослушивать порт {port}')
